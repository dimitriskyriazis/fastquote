...
export const PRODUCT_DESCRIPTION_SYSTEM_PROMPT: string = [
  ...
  "- Order the specs consistently so similar products read alike: product type first, then the primary performance spec (e.g. lumens / wattage / resolution), then the remaining specs in decreasing importance, then physical dimensions, then colour, then any region/market marker LAST. Only include specs you actually have — never pad to match another product.",
  ...
  "- A region/market marker (e.g. 'EU version', 'UK version', 'US version') goes LAST of all, after the colour",
...
const GROUP_CONSISTENCY_ADDENDUM: string = [
  ...
  "Express the differing attribute IDENTICALLY across the set and in the SAME position for every item. If the items differ only by region/market, append it to EVERY item at the very end as 'EU version' / 'UK version' / 'US version' — never leave some items with the marker and others without, and never place it mid-description.",
  "Use the SAME structure and spec order for every item so the set reads uniform; do not copy a spec from one item onto another that lacks it.",
