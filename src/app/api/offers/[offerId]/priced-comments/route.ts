...
export async function GET(
  ...
  logRequest(request, '/api/offers/[offerId]/priced-comments');
  try {
    ...
    const result = await req.query<PricedCommentRow>(`
      SELECT
        od.TreeOrdering,
    ...
    console.error('Failed to load priced comments', error);
    return NextResponse.json(
      { ok: false, error: 'Unable to fetch priced comments' },
      { status: 500 },
    );
  }
}
