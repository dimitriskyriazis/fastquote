...
const COLUMN_EXPRESSIONS: Record<string, string> = {
  MailContactGroupID: "mcg.ID",
  ContactGroupID: "cg.ID",
  Description: "cg.Description",
  TotalCount: "cg.TotalCount",
  MinimumImportance: "mcg.MinimumImportance",
  Note: "mcg.Note",
};
...
const IMPORTANCE_SORT_COLUMNS = new Set(["MinimumImportance"]);
...
export async function POST(
  ...
  logRequest(req, '/api/marketing/mails/[mailId]/contact-groups');
  try {
    ...
    const selectSql = `
      SELECT
        ...
        mcg.ID AS MailContactGroupID,
        cg.ID AS ContactGroupID,
        cg.Description,
        cg.TotalCount,
        mcg.MinimumImportance,
        mcg.Note
      FROM dbo.MailContactGroups mcg
      INNER JOIN dbo.ContactGroups cg ON cg.ID = mcg.ContactGroupID
      ${combinedWhere}
      ${orderClause}
...
export async function PATCH(
  ...
  try {
    ...
    await params; // validate mailId exists in URL
    ...
    for (const update of updates) {
      const mcgId = Number(update.MailContactGroupID);
      if (!Number.isFinite(mcgId)) continue;
      const field = String(update.field ?? "");
      const value = update.value;

      const request = pool.request();
      request.input("mcgId", sql.Int, mcgId);

      if (field === "MinimumImportance") {
        request.input("value", sql.NVarChar(50), value != null ? String(value).trim() : null);
        await request.query(`UPDATE dbo.MailContactGroups SET MinimumImportance = @value WHERE ID = @mcgId`);
      } else if (field === "Note") {
        request.input("value", sql.NVarChar(sql.MAX), value != null ? String(value).trim() : null);
        await request.query(`UPDATE dbo.MailContactGroups SET Note = @value WHERE ID = @mcgId`);
      }
    }
...
export async function DELETE(
  ...
  try {
    ...
    const body = (await req.json().catch(() => null)) as { MailContactGroupIDs?: Array<number | string> } | null;
    const rawIds = Array.isArray(body?.MailContactGroupIDs) ? body.MailContactGroupIDs : [];
    const ids = rawIds
    ...
    await request.query(`
      DELETE FROM dbo.MailContactGroups
